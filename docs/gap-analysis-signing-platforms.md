...
| **Drop-in Linux replacement for AzureSignTool** | Partial | **`psign-tool portable sign-pe --azure-key-vault-*`** and **`psign-tool --mode portable sign --azure-key-vault-*`** can build and embed PE Authenticode signatures with Key Vault RSA signing. **`azure-key-vault-sign-digest`** remains available for lower-level **`keys/sign`** workflows. Gaps: sign-time timestamp URL is not wired into portable signing yet, and non-PE remote-sign embedding still requires Windows mode or future portable signer support. |
